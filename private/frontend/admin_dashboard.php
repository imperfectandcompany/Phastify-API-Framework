...
<div class="container mx-auto px-4">
    ...
    <form method="get" class="mb-8">
        <div class="flex flex-col md:flex-row md:space-x-4">
            <input type="text" name="query" id="query" placeholder="Search..."
                class="w-full md:w-auto p-2 border border-gray-300 rounded mb-2 md:mb-0 focus:outline-none"
                value="<?php echo isset($_GET['query']) ? htmlspecialchars($_GET['query']) : ''; ?>" />
            <select name="category" id="category"
                class="w-full md:w-auto py-2 px-4 border border-gray-300 rounded focus:outline-none">
            ...
            <button type="submit"
                class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition w-full md:w-auto">Search</button>
        </div>
    </form>
            ...
            try {
                ...
                if (is_array($searchResults)) {
                    ...
                    foreach ($searchResults as $result) {
                        echo '<li class="mb-2">';
                        // Display 'Type'
                        echo '<span class="font-semibold">Type:</span> ' . $result['type'] . '<br>';

                        // Switch content based on type
                        switch ($result['type']) {
                            case 'post':
                                echo '<span class="font-semibold">Post content:</span> ' . ($result['content'] ?? '');
                                break;
                            case 'comment':
                                echo '<span class="font-semibold">Comment content:</span> ' . ($result['content'] ?? '');
                                break;
                            case 'user':
                                echo '<span class="font-semibold">Username:</span> ' . ($result['content'] ?? '');
                                break;
                            case 'integration':
                                echo '<span class="font-semibold">Service Name:</span> ' . ($result['content'] ?? '');
                                break;
                            case 'reply':
                                echo '<span class="font-semibold">Reply content:</span> ' . ($result['content'] ?? '');
                                break;
                            default:
                                echo 'Unknown Type';
                        }
    ...
    <div class="mb-8 text-right">
        <form method="post">
            ...
            <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition w-full md:w-auto"
