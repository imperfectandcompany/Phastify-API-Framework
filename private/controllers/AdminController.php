...
class AdminController
{
    ...
    // TODO: Only save cookie on admin domain and provide necessary redirect
                ...
                if (isset($_COOKIE['POSTOGONADMINID_'])) {
                    ...
                    if ($deviceId) {
                        ...
                        if (!$roles->isUserAdmin($userid)) {
                            ...
                            // 3 day expiry time
                            setcookie("POSTOGONADMINID", $token, time() + 60 * 60 * 24 * 3, '/', $GLOBALS['config']['url'], TRUE, TRUE);
                            ...
                            setcookie("POSTOGONADMINID_", '1', time() + 60 * 60 * 24 * 1, '/', $GLOBALS['config']['url'], TRUE, TRUE);
                            $isLoggedIn = true;
                            $isAuthorized = true;
                            return $userid;
                        }
    ...
    public function loadAdminLogin()
    {
        if($this->isAdminLoggedIn()){
            header("Location: ".$GLOBALS['config']['url']."/admin/dashboard");
        }
        ...
        ob_start();
        include_once($GLOBALS['config']['private_folder'] . '/frontend/admin_login.php');
        $var=ob_get_contents();
        ob_end_clean();
        echo $var;
    }
    ...
    public function loadAdminSearch()
    {
        ...
        if(!$uidIsLoggedInAuthorized){
            ...
            header("Location: ".$GLOBALS['config']['url']."/admin/login");
        }
        $prodLogger = new Logger($this->prodDbConnection);
